...
import { Package, Plus, Search, Pencil, Sparkles } from 'lucide-react';
...
const AdminSuppliersPage = () => {
  ...
  const handleSmartImport = async (data: any) => {
    if (data.entity) {
      const e = data.entity;
      const payload = {
        name: e.name || 'Fornecedor importado',
      ...
      const { data: inserted } = await supabase.from('suppliers').insert(payload as any).select().single();
      if (inserted && data.services?.length > 0) {
        const servicesPayload = data.services.map((s: any) => ({
          supplier_id: (inserted as any).id,
          name: s.name || 'Serviço',
      ...
      toast({ title: 'Fornecedor importado', description: `${data.services?.length || 0} serviço(s)` });
      fetchSuppliers();
    }
  };
